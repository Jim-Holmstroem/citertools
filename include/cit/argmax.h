...
/*
 * argmax, takes a measure functor that returns a type that can be used to compare elements
...
namespace cit {
    ...
    inline typename ContainerT::const_iterator argmax(const ContainerT & container,
                                                      const MeasureFunctorT & functor = MeasureFunctorT()) {
    ...
    inline T argmax(std::initializer_list<T> l, const MeasureFunctorT & functor = MeasureFunctorT()) {
        return *argmax(l.begin(), l.end(), functor);
    }
